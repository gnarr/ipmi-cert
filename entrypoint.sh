...
echo "$CRON_STRING cd /app && $POETRY_PATH run python /app/main.py --ipmi-url \$IPMI_URL --key-file \$KEY_FILE --cert-file \$CERT_FILE --username \$USERNAME --password \$PASSWORD $NO_REBOOT_FLAG $DEBUG_FLAG >> /proc/1/fd/1 2>> /proc/1/fd/2" | crontab -
